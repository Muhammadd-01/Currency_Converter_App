              ...
              {loading ? (
                ...
                filteredUsers.map((user) => (
                  <tr key={user.uid} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${user.customClaims?.superAdmin
                          ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/20'
                          : user.customClaims?.admin
                            ? 'bg-blue-500/20 text-blue-400 border-blue-500/20'
                            : 'bg-gray-500/20 text-gray-400 border-gray-500/20'
                        }`}>
                        {user.customClaims?.superAdmin ? 'Super Admin' : user.customClaims?.admin ? 'Admin' : 'User'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
